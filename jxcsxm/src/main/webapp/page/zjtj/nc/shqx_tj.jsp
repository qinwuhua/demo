...
// 			loadBmbm('bd','标段');
			...
			$.ajax({
				...
				success:function(data){
					$.each(data,function(index,item){
						if(item.sfds=='1'){
							$('#zjtj_table_tbody').append('<tr name="'+$.cookie("unit")+'" id="'+item.id+'"><td align="center">'+item.name+'</td><td align="center"><input type="number" width="60" readonly="readonly"/><font color="red"></font></td><td  align="center"><input type="number" width="60" readonly="readonly"/><font color="red"></font></td><td  align="center"><input type="number" width="60" readonly="readonly"/><font color="red"></font></td><td  align="center"><input type="number" width="60" readonly="readonly"/><font color="red"></font><input type="hidden" value="'+item.parent+'"/><input type="hidden" value="'+item.sfds+'"/><input type="hidden" value="'+item.sftj+'"/></td></tr>');
						}else{
							if(item.sftj=='1'){
								$('#zjtj_table_tbody').append('<tr class="'+item.parent+'" name="'+$.cookie("unit")+'" id="'+item.id+'"><td align="center">'+item.name+'</td><td align="center"><input type="number" width="60" onchange="jssjzj('+item.parent+')"/><font color="red"></font></td><td  align="center"><input type="number" width="60" onchange="jssjzj('+item.parent+')"/><font color="red"></font></td><td  align="center"><input type="number" width="60" onchange="jssjzj('+item.parent+')"/><font color="red"></font></td><td  align="center"><input type="number" width="60" onchange="jssjzj('+item.parent+')"/><font color="red"></font><input type="hidden" value="'+item.parent+'"/><input type="hidden" value="'+item.sfds+'"/><input type="hidden" value="'+item.sftj+'"/></td></tr>');
							}else{
								$('#zjtj_table_tbody').append('<tr name="'+$.cookie("unit")+'" id="'+item.id+'"><td align="center">'+item.name+'</td><td align="center"><input type="number" width="60" /><font color="red"></font></td><td  align="center"><input type="number" width="60" /><font color="red"></font></td><td  align="center"><input type="number" width="60" /><font color="red"></font></td><td  align="center"><input type="number" width="60" /><font color="red"></font><input type="hidden" value="'+item.parent+'"/><input type="hidden" value="'+item.sfds+'"/><input type="hidden" value="'+item.sftj+'"/></td></tr>');
							}
						}
					});
				}
		...
		function loadZj(gydwdm){
			...
			else zj={'gydwdm':$.cookie('unit'),'bfyf':$("#nf").combo('getValue')+"-"+$("#yf").combo('getValue'),'nf':$("#nf1").combo('getValue'),'jhxdwh':$("#jhxdwh").combo('getValue')};
			$.ajax({
				type:'post',
				async:false,
				url:'/jxcsxm/zjtj/queryZjByGydwdm.do',
				data:zj,
				dataType:'json',
				success:function(data){
					if(data.length>0){
		...
		function loadxddw(gydwdm){
			...
			$.ajax({
				type:'post',
				async:false,
				url:'/jxcsxm/zjtj/queryZjdwbfByGydwdm.do',
				data:zj,
				dataType:'json',
				success:function(data){
					if(data.length>0){
		...
		function save(){
			...
			$.ajax({
				type:'post',
				url:'/jxcsxm/zjtj/insertOrUpdateShqx.do',
        		...
        		<td>
        			<div align="center">
        				...
        				<tr align='center' height="28">
								...
								<input value=" 查询  " onclick="search()" style="text-align: center;" type="button"/><br><font color="red">*选择年份、调剂月份、计划下达文号后点击查询自动计算剩余资金</font>
							</td>
						</tr>
        				
        				<tr align='center' height="28">
        						<td width="100">调剂月份</td>
						...
						<table id="zjtj_table" width="800" class="table" cellpadding="0" cellspacing="0">
							...
							<tbody id="zjtj_table_tbody"></tbody>
						</table>
						<table width="600">
							<tr align="center">
								<td>
